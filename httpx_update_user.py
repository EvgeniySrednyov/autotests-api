import httpx
from tools.fakers import fake


BASE_URL = 'http://localhost:8000'

#Создание пользователя
create_user_payload = {
      "email": fake.email,
      "password": "test",
      "lastName": "string",
      "firstName": "string",
      "middleName": "string"
}

create_user_response = httpx.post(f'{BASE_URL}/api/v1/users', json=create_user_payload)
create_user_response_data = create_user_response.json()
print('StatusCode создания пользователя:', create_user_response.status_code)
print(create_user_response_data)

#Аутентификация
login_payload = {
    "email": create_user_payload['email'],
    "password": create_user_payload['password']
}

login_response = httpx.post(f'{BASE_URL}/api/v1/authentication/login', json=login_payload)
login_response_data = login_response.json()
print('StatusCode аутентификации:', login_response.status_code)

#Обновление пользователя
update_user_payload = {
      "email": fake.email,
      "password": "test",
      "lastName": "string",
      "firstName": "string",
      "middleName": "string"
}

update_user_headers = {
    "Authorization": f"Bearer {login_response_data['token']['accessToken']}"
}

update_user_response = httpx.patch(
    f'{BASE_URL}/api/v1/users/{create_user_response_data['user']['id']}',
    headers=update_user_headers,
    json=update_user_payload
)
update_user_response_data = update_user_response.json()
print('StatusCode обновления пользователя:', update_user_response.status_code)
print(update_user_response_data)
